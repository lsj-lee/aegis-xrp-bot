...
# Changed to a list for safer subprocess execution
...
# Define log file path within the project root
LOG_FILE = PROJECT_ROOT / "aegis_system.log"

# Get a logger instance
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO) # Set default logging level to INFO

# Create file handler, set level, and add formatter
...
# Create console handler, set level, and add formatter
console_handler = logging.StreamHandler(sys.stdout)
console_handler.setLevel(logging.INFO) # Console will also log INFO and above
console_handler.setFormatter(formatter)
logger.addHandler(console_handler)

# Remove the custom print_status function as logging module handles this now.
...
def check_pending_proposals():
    ...
    If found, logs a message with instructions for review and approval.
    """
    if EVOLUTION_QUEUE_DIR.exists() and EVOLUTION_QUEUE_DIR.is_dir():
        ...
        if proposals:
            ...
            logger.info(f"👉 'python aegis_system_evolver.py --review' 명령어로 검토 및 승인하세요.")
            logger.info("-" * 65)
...
def _execute_pipeline_step(script_name: str, step_msg: str) -> bool:
    ...
    Captures and logs subprocess stdout/stderr for better error diagnosis.
    ...
    if not script_path.exists():
        logger.error(f"⚠️ 오류: {script_path} 파일을 찾을 수 없습니다. 파이프라인을 중단합니다.")
        return False
    if not script_path.is_file():
        logger.error(f"⚠️ 오류: {script_path}은(는) 파일이 아닙니다. 파이프라인을 중단합니다.")
        return False
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(f"⚠️ {script_name} 실행 중 오류 발생 (Exit Code: {result.returncode})")
            if result.stdout:
                logger.error(f"--- {script_name} STDOUT ---\n{result.stdout.strip()}")
            if result.stderr:
                logger.error(f"--- {script_name} STDERR ---\n{result.stderr.strip()}")
            logger.error(" 파이프라인을 중단합니다.")
            return False
        else:
            # Optionally, log subprocess output even on success for detailed debugging.
            # For typical production runs, this might be too verbose.
            # You can change logger.debug to logger.info if verbose success output is desired.
            if result.stdout:
                logger.debug(f"--- {script_name} STDOUT ---\n{result.stdout.strip()}")
            if result.stderr: # Should ideally be empty on success
                logger.debug(f"--- {script_name} STDERR ---\n{result.stderr.strip()}")
            pass

    except FileNotFoundError:
        # This specific error indicates that PYTHON_EXEC itself could not be found.
        logger.critical(f"⚠️ 실행 실패: '{PYTHON_EXEC}'을(를) 찾을 수 없습니다.")
        logger.critical("    가상 환경이 설정되었는지 또는 '.venv' 경로가 올바른지 확인하세요. 파이프라인을 중단합니다.")
        return False
    except Exception as e:
        # Catch any other unexpected errors during subprocess execution.
        logger.critical(f"⚠️ 예기치 못한 오류 발생 ({script_name}): {e}. 파이프라인을 중단합니다.", exc_info=True)
        return False
...
def _enter_sleep_mode():
    ...
    Uses subprocess.run for better error handling.
    """
    if sys.platform == 'darwin': # Check if the operating system is macOS
        ...
        try:
            # Use subprocess.run with the list version of the command
            result = subprocess.run(MAC_SLEEP_COMMAND, check=True, capture_output=True, text=True)
            if result.returncode == 0:
                logger.info("✅ 맥북이 성공적으로 절전 모드로 전환되었습니다.")
            else:
                # This block is theoretically unreachable if check=True and a non-zero exit code occurs,
                # as CalledProcessError would be raised instead.
                logger.warning(f"⚠️ 맥북 절전 모드 전환 실패 (Exit Code: {result.returncode}).")
                if result.stderr:
                    logger.warning(f"   STDERR: {result.stderr.strip()}")
                logger.warning("   수동으로 전환해주세요.")
        except FileNotFoundError:
            logger.error(f"⚠️ '{MAC_SLEEP_COMMAND[0]}' 명령어를 찾을 수 없습니다. macOS 환경이 아니거나, 'pmset'이 설치되지 않았을 수 있습니다. 수동으로 전환해주세요.")
        except subprocess.CalledProcessError as e:
            logger.error(f"⚠️ 맥북 절전 모드 전환 실패: 명령어 실행 중 오류 발생 (Exit Code: {e.returncode}).")
            if e.stderr:
                logger.error(f"   STDERR: {e.stderr.strip()}")
            logger.error("   수동으로 전환해주세요.")
        except Exception as e:
            logger.error(f"⚠️ 예기치 못한 오류로 맥북 절전 모드 전환 실패: {e}. 수동으로 전환해주세요.", exc_info=True)
    else:
        logger.info("⚠️ 현재 운영체제는 macOS가 아닙니다. 절전 모드 기능을 건너뜁니다.")
...
def run_pipeline(auto_sleep: bool = False):
    ...
    logger.info(f"🐍 파이썬 실행기: {PYTHON_EXEC}")
    logger.info(f"💤 자동 수면 모드: {'ON' if auto_sleep else 'OFF'}")
    logger.info("="*65)

    # Validate Python executable early to prevent later failures.
    if not PYTHON_EXEC.exists():
        logger.critical(f"❌ 오류: 지정된 파이썬 실행기 '{PYTHON_EXEC}'을(를) 찾을 수 없습니다.")
        logger.critical("    가상 환경이 활성화되었는지, 또는 .venv 경로가 올바른지 확인하세요. 파이프라인 종료.")
        sys.exit(1) # Exit with an error code
    if not PYTHON_EXEC.is_file():
        logger.critical(f"❌ 오류: '{PYTHON_EXEC}'은(는) 실행 가능한 파일이 아닙니다. 파이프라인 종료.")
        sys.exit(1) # Exit with an error code
    ...
    logger.info("\n=================================================================")
    logger.info(f"✅ 모든 시스템이 성공적으로 완료되었습니다! (총 소요시간: {elapsed_minutes:.1f}분)")
    logger.info("🎯 이제 구글 시트에서 AI의 '오늘자 성적표'를 확인하세요.")
    logger.info("=================================================================")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable-sleep", 
        action="store_true", 
        help=f"Enables automatic sleep mode ({SLEEP_WAIT_SECONDS}s wait, then '{' '.join(MAC_SLEEP_COMMAND)}') after successful pipeline completion." # Updated help text
    )
    ...
    try:
        ...
        # Catch any unexpected top-level exceptions in the pipeline execution
        logger.critical(f"🚨 시스템 파이프라인 실행 중 치명적인 예외 발생: {main_e}", exc_info=True)
