...
def get_gemini_insight(data_dict, am):
    ...
    try:
        ...
        The model MUST be based on Transformer/TFT architecture and optimized for MacBook M5 MPS acceleration.
        Consider using Residual connections, Attention mechanisms specialized for financial time-series.
        IMPORTANT: Provide ONLY the executable Python code block. Do not add markdown backticks.
        The class must inherit from nn.Module and include an __init__ method taking 'input_size'.
        """
        res_2 = client.models.generate_content(model='gemini-2.5-flash', contents=prompt_step_2).text

        # 💾 Save Generated Code Proposal
        try:
            ...
            with open("aegis_evolution_proposal.py", "w") as f:
        ...
        [🧬 에이지스 진화 연구]: Explain the architectural improvements in the generated code (Step 2) in detail (Korean). Discuss Transformer, TFT, and MPS optimization.
        [💻 진화 코드 제안]: Display the 'AegisEvolution' class code from Step 2.
